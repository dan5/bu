...
describe "RequestedGroups" do
  context "あなたがオーナーではないグループの場合" do
    ...
    let!(:group) { FactoryGirl.create(:group, owner_user_id: other, users: [other], permission: 1) }
  ...
  context "あなたがオーナーのグループの場合" do
    ...
    let!(:group) { FactoryGirl.create(:group, owner_user_id: you, users: [you], permission: 1) }
